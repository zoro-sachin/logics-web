...
const TimeAttackLogic = ({ difficulty = 'medium' }) => {
    ...
    const handleAnswer = (answer) => {
        ...
        if (isCorrect) {
            ...
            const points = currentQuestion.points * streakMultiplier;
        ...
        try {
            await submitGame({
                ...
                gameData: {
                    ...
                    maxStreak: Math.max(...answers.map((_, i) => {
                        let s = 0;
                        for (let j = i; j >= 0 && answers[j].isCorrect; j--) s++;
                        return s;
                    }), 0)
                }
            });
                        ...
                        <div className="answer-options">
                            {currentQuestion.options.map((option, index) => {
                                ...
                                if (selectedAnswer === option) {
                                    className += option === currentQuestion.correctAnswer
                                        ? ' correct'
                                        : ' wrong';
                                }
