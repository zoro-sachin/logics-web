...
const generateQuestion = async (req, res) => {
    try {
        ...
        const questionData = await questionService.getQuestionFromBank(targetCategory, targetDifficulty, targetLevel);

        if (!questionData) {
            return res.status(404).json({
                success: false,
                message: 'No questions available in bank for this category'
            });
        }
